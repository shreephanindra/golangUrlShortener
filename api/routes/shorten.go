...
func ShortenURL(c *fiber.Ctx) error {
	...
	if err == redis.Nil {
		...
		if valInt <= 0 {
			...
			return c.Status(fiber.StatusServiceUnavailable).JSON(fiber.Map{"error": "Limit Exceeded",
	...
	if !helper.RemoveDomainError(body.URL) {
		return c.Status(fiber.StatusServiceUnavailable).JSON(fiber.Map{"error": "Can't Acess"})
	}
